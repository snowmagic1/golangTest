...
func (sm *ShardMaster) ApplyToStateMachine(applyOp Op) (reply interface{}) {
	...
	switch applyOp.Code{
		case Join:
			...
			if(currGroupLen + len(serversMap) > NShards) {
				joinReply.Err = Err_InvalidParameter
				reply = joinReply
				break
			}	
			...
			for groupId, servers := range serversMap {
				...
				if(ok) {
					log += fmt.Sprintf("server, groupId [%v] already exists\n", groupId)
					continue
				}
				...
				for i:=0;i<NShards;i++ {
					if( i%(currGroupLen + 1) == 0) {
						currShards[i] = groupId
						log += fmt.Sprintf("server, assign shard [%v] to [%v]\n", i, groupId)
					}	
				}
			}
		...
		case Leave:
			...
			for _, gid := range gids {
				...
				if(!ok) {
					log += fmt.Sprintf("server, groupId [%v] not exists\n", gid)
					continue
				}
				...
				for i:=1;i<NShards;i++ {
					if( currShards[i] == gid) {
						currShards[i] = firstGid
						log += fmt.Sprintf("server, shard [%v] moves to [%v]\n", i, firstGid)
					}	
				}
			}
		...
		case Move:
			...
			if(!ok) {
				...
				reply = moveReply
				log += fmt.Sprintf("server, Move, groupId [%v] not exists\n", gid)
			...
			if(shard < 0 || shard >= NShards) {
				moveReply.Err = Err_InvalidParameter
				reply = moveReply
				log += fmt.Sprintf("server, Move, invalid shard [%v]\n", shard)
